        ...
        <form action="<?php echo Url::to(MODULE_ADMIN."/".$params["cName"]."/operation")?>" method="post">
            ...
            <div class="panel-body">
                <div class="margin-bottom-10">
                    <label class="pull-left margin-top-6 margin-right-10">Əsas kursu təyin et:</label>
                    <select onChange="window.location.href=this.value" class="form-control" style="width:auto;">
                        <?php
                        $languages = CurrenciesModel::getCurrencies();
                        foreach($languages as $lang){
                            if($lang["default"]==1) $selected='selected="selected"'; else $selected='';
                            echo '<option value="'.Url::to(MODULE_ADMIN.'/'.$params["cName"].'/setdefaultcurrency/'.$lang["id"]).'" '.$selected.'>'.$lang["fullname"].'</option>';
                        }
                        ?>
                    </select>
                </div>
