...
use Modules\panadmin\Models\CategoriesModel as TableModel;
...
class Categories extends MyController
{
    ...
    protected function getImageUpload($id,$createStatus=false)
    {
        if(!empty($_FILES['image']['tmp_name']) and Security::filterFileMimeTypes($_FILES['image']['type'])) {
            ...
            Database::get()->update($this->dataParams["cName"], ['image' => $sql_img], ['id' => $id]);
        }
    }
...
//        $cache = new FileCache();
//
//        $data = $cache->get("kateqoriyalar1");
//
//        if($data==false){
//            echo "salam<br />";
//            $data = Database::get()->selectOne("SELECT * FROM `categories`");
//            $cache->save("kateqoriyalar1",$data,60);
//        }
//
//        var_dump($data);
//
