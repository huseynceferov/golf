...
use Modules\panadmin\Models\NewsModel as TableModel;
...
class News extends MyController
{
    ...
    protected function getImageUpload($id,$createStatus=false)
    {
        if(!empty($_FILES['image']['tmp_name']) and Security::filterFileMimeTypes($_FILES['image']['type'])) {
            ...
            $img->load($_FILES["image"]["tmp_name"])->resize(272, 181)->save($destination);
            $img->load($_FILES["image"]["tmp_name"])->resize(572, 350)->save($destination_middle);
            ...
            Database::get()->update($this->dataParams["cName"], ['image' => $sql_img, 'middle' => $sql_middle_img, 'thumb' => $sql_thumb_img], ['id' => $id]);
        }
    }
