...
class Clubs extends MyController
{
    ...
    public static $imageFolder = 'clubs';
    ...
    protected function getImageUpload($id,$createStatus=false)
    {
        if(!empty($_FILES['image']['tmp_name']) and Security::filterFileMimeTypes($_FILES['image']['type'])) {
            ...
            $sql_img_middle = self::$imageFolder.'/'.$id.'/middle/' . $id."_0.".$ext;
            $sql_thumb_img = self::$imageFolder.'/'.$id.'/thumbs/' . $id."_0.".$ext;
            Database::get()->update($this->dataParams["cName"], ['image' => $sql_img, 'middle'=>$sql_img_middle, 'thumb' => $sql_thumb_img], ['id' => $id]);
        }
    }
