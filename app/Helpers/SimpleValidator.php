...
use Helpers\SimpleValidatorException;
...
class SimpleValidator {
    ...
    protected function getDefaultLang() {
        return LanguagesModel::defaultLanguage();
    }
    ...
    protected function getDefaultErrorTexts($lang = null) {
        ...
        $errorLangFile = "app/language/".$lang."/validator-error/validation.php";

        if (file_exists($errorLangFile)) {
            $default_error_texts = include($errorLangFile);
        }
            ...
            if (is_array($input_rules)) {
                foreach ($input_rules as $rule => $closure) {
                    if (!isset($inputs[(string) $input]))
                        ...
                        $input_value = $inputs[(string) $input]; // javid edit
                    /**
    ...
    protected static function http_check($input){
        return (parse_url($input, PHP_URL_SCHEME));
    }
    ...
    protected static function between($input, $param1, $param2)
    {
        return (mb_strlen($input, 'utf-8') >= $param1 && mb_strlen($input, 'utf-8') <= $param2);
    }

    protected static function password($input)
    {
        return (preg_match("/^(?=.*\d)(?=.*[a-zA-Z])[0-9A-Za-z!#@%$]{6,15}$/", $input));
    }

    protected static function check_login($input)
    {
        return (preg_match("#^[a-zA-Z0-9_-]+$#", $input) == 1);
    }

    protected static function exists($input, $tableName, $field, $id)
    {
        $sql = "SELECT `id`, `".$field."` FROM `".$tableName."` WHERE `".$field."` = '".$input."' LIMIT 1";
        $query = Database::get()->selectOne($sql);

        if (is_array($query) && count($query) > 0 && $query['id'] != $id)
            return false;
        return true;
    }

    protected static function check_date($input)
    {
        $date = date('d.m.Y', strtotime($input));
        if($date == $input) {
            return true;
        } else {
            return false;
    ...
    protected static function min_number($input, $param)
    {
        return (is_numeric($input) && $input >= $param);
    }

    protected static function max_number($input, $param)
    {
        return (is_numeric($input) && $input <= $param);
    }

    protected static function between_number($input, $param1, $param2)
    {
        return (is_numeric($input) && $input >= $param1 && $input <= $param2);
    }

    protected static function required_file($input)
    {
        //print_r($input); exit;
        return (!empty($input['name']) || !empty($input['tmp_name']));
    }

    protected static function image_mime_types($input, $param1)
    {
        $param1 = str_replace(array('-'), ',', $param1);
        $params = explode(",", $param1);

        foreach ($params as $keys) {
            $mime_types[] = "image/".$keys;
        }

        return in_array($input['type'], $mime_types);
    }
