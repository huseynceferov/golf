...
class Recursive {
    ...
    public static function menu($db, $tablename, $root_id = 0, $suffix, $place, $static_url = 'page', $mainUlClass='dropdown-menu', $mainLiClass = 'dropdown custom-drop', $otherLiClass='dropdown-submenu', $main_a_class = 'scroll easing nav-click-hide'){
        global $full_url;
        ...
        while ( $loop && ( ( $option = each( $children[$parent] ) ) || ( $parent > $root_id ) ) ){
            if ( $option === false ){
                ...
                $html[]= '<li '.$active_class.'><a href="'.$link.'" class="'.$main_a_class.'">';
                if (isset($cPath_array) && in_array($option['value']['id'], $cPath_array)) {
                    $html[]='<strong>'.stripslashes($option['value']['title']).$pic.'</strong>';
                }else{
                    $html[]=stripslashes($option['value']['title']).$pic;
