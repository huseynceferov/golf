...
class Pagination
{
    ...
    public $options = [15, 25, 50, 100, 'all' => 'Hamısı'];
    ...
    public function pageNavigation($custom_pagination = 'pagination')
    {
        ...
        if($this->currentPage>1)
            $return.= '<li><a href="'.Url::to(Url::addFullUrl(["page" => ($this->currentPage-1)])).'" aria-label="Previous" class="prev"><i class="fa fa-angle-double-left" aria-hidden="true"></i></a></li>';
        for($i=$this->currentPage-$this->showButtonCount;$i<=$this->currentPage+$this->showButtonCount;$i++)
        {
            ...
            if($i>0 && $i<=$this->maxPage)
                $return.= '<li><a '.$class.' href="'.$href.'">'.$i.'</a></li>';
        }
        if($this->currentPage<$this->maxPage)
            $return.= '<li><a href="'.Url::to(Url::addFullUrl(["page" => ($this->currentPage+1)])).'" aria-label="Next" class="next"><i class="fa fa-angle-double-right" aria-hidden="true"></i></a></li>';
    ...
    public static function getCountData($countRows=0,$startRow=0,$limitRow=0)
    {
        ...
        $return =  'The total number of data : <b>'.$countRows.'</b>, is displayed: <b>'.$startRow.'-'.($startRow+$limitRow-1).'</b>';
        return $return;
    }
    ...
    public function getLimitSelector()
    {
        ...
        $return = '<b class="text-info">Show on each page</b> <select class="padding-4 border-radius-5"  onchange="location = this.value;">
                    '.$optionsField.'
                </select>';
        return $return;
    }
