...
class Session
{
     ...
     * @return string
     */
    public static function setFlash($sessionName,$msg)
    {
         if (isset($msg) and !empty($msg)) {

             self::set('flash_message',['message' => $msg,"type" => $sessionName]);
    ...
    public static function getFlash()
    {
        ...
        if($session["message"]!=''){
            if($session["type"]=='error') {
                $session["type"] = 'danger';
                $session["icon"] = 'fa-exclamation';
            }else{
                $session["type"] = 'success';
                $session["icon"] = 'fa-check';
            }

            return '<div class="alert alert-'.$session["type"].' alert-dismissable margin-bottom-3 margin-top-10">
